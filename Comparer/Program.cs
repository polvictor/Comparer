...
class Program
{
    ...
    static public bool PersonFirstnameComparerUp(object o1, object o2)
    {
        Person left = o1 as Person;
        Person right = o2 as Person;
    ...
    static public bool PersonDateComparerYoungToOld(object o1, object o2)
    {
        Person left = o1 as Person;
        Person right = o2 as Person;
        if (left == null || right == null)
    ...
    static void Main()
    {
        ...
        Console.WriteLine("\nВiдсортований список по датi народження (за вiком вiд старших до молодших):\n");
        Sorter.BubbleSort(persons, PersonDateComparerOldToYoung); // сортируем от старших по возрасту к младшим
        foreach (object person in persons) Console.WriteLine(person);

        Console.WriteLine("\nВiдсортований список по датi народження (за вiком вiд молодших до старших):\n");
        Sorter.BubbleSort(persons, PersonDateComparerYoungToOld); // сортируем от младших по возрасту к старшим
        foreach (object person in persons) Console.WriteLine(person);

        Console.WriteLine("\nВiдсортований за прiзвищами (А-Я) список:\n");
        Sorter.BubbleSort(persons, PersonLastnameComparer); // сортируем по фамилиям
        foreach (object person in persons) Console.WriteLine(person);

        Console.WriteLine("\nВiдсортований за прiзвищами (Я-А) список:\n");
        Sorter.BubbleSort(persons, PersonLastnameComparerUp); // сортируем по фамилиям
        foreach (object person in persons) Console.WriteLine(person);

        Console.WriteLine("\nВiдсортований список за довжиною прiзвища:\n");
        Sorter.BubbleSort(persons, PersonLastnameLengthComparer); // сортируем по длине фамилии
        foreach (object person in persons) Console.WriteLine(person);
        ...
        Console.WriteLine("\nВiдсортований за iменами (Я-A) список:\n");
        Sorter.BubbleSort(persons, PersonFirstnameComparerUp); // сортируем по именам
        foreach (object person in persons) Console.WriteLine(person);
